...
description: Advanced operational skill for domain-moat-narrative-builder.
read_when:
  - When working in scale-systems workflows
  - When user asks for domain moat narrative builder
  - When a structured artifact must be generated
  - When reusable documentation is required
metadata: {"codex": {"level": "advanced", "category": "scale-systems", "runtime": "markdown+python", "requires": {"files": ["templates/", "scripts/", "examples/"]}}}
---

# Domain Moat Narrative Builder

    Advanced operational skill for domain-moat-narrative-builder.

    ## Activation trigger

    Use this skill when the user requests **domain moat narrative builder** and needs a high-confidence, decision-ready outcome in **scale-systems**.
...
- If high-risk uncertainty remains, return a gated recommendation instead of false precision.
    ...
    ```shell
    $domain-moat-narrative-builder "Run domain moat narrative builder on my current project context"
